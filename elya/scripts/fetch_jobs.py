#!/usr/bin/env python3
"""
Collecte les offres d'emploi pour Elya Ranc (TSSR) depuis des sources PUBLIQUES.

Sources :
  - France Travail → scraping HTML
  - LinkedIn       → guest API publique

Les offres sont scorées par rapport au profil (elya/profile.json) et
sauvegardées dans elya/data/jobs.json pour le dashboard GitHub Pages.
"""
...
def score_job(job, profile):
    ...
    # Contrat — 10 pts (tous types acceptés par Elya)
...
def fetch_france_travail(profile) -> list:
    ...
    search_queries = [
        f"technicien système réseaux {dept}",
        f"technicien informatique {city}",
        f"support informatique {city}",
        f"helpdesk {dept}",
        f"administrateur réseaux {dept}",
        f"technicien réseau {dept}",
        f"TSSR {dept}",
        f"technicien poste travail {dept}",
        f"Active Directory {dept}",
        f"administrateur systèmes {dept}",
    ]
...
def fetch_linkedin(profile) -> list:
    ...
    search_queries = [
        ("technicien systeme reseaux",          f"{city}, Gironde, France"),
        ("support informatique helpdesk",        f"{city}, Gironde, France"),
        ("administrateur reseaux systemes",      f"{city}, Gironde, France"),
        ("technicien informatique",              f"{city}, France"),
        ("TSSR technicien reseau",               f"{city}, France"),
        ("administrateur Active Directory",      f"{city}, France"),
    ]
...
def main():
    ...
    print(f"\n✓ {len(all_jobs)} offres sauvegardées dans elya/data/jobs.json")
